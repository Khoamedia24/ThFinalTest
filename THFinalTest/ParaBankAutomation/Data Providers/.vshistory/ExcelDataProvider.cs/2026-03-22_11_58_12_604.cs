using OfficeOpenXml;
using TestCaseModel = THFinalTest.ParaBankAutomation.Models.TestCaseData;
using TestStepModel = THFinalTest.ParaBankAutomation.Models.TestStep;
...
namespace DoAnCuoiKy_TH.DataProviders
{
    public class ExcelDataProvider
    {
        ...
        private const string DefaultExcelFileName = "Testcase.xlsx";
        public const string PlaceholderTestCaseId = "__MISSING_OR_EMPTY_TEST_DATA__";
        private const int DataStartRow = 7;

        // 2. Cấu hình tọa độ các cột (Tính theo bảng chữ cái A=1, B=2, C=3...)
        private const int TestCaseIdColumn = 2;   // Cột B

        private const int FunctionColumn = 3;     // Cột C
        private const int BigItemColumn = 4;      // Cột D
        private const int MediumItemColumn = 5;   // Cột E
        private const int SmallItemColumn = 6;    // Cột F
        private const int PreConditionColumn = 7; // Cột G
        private const int StepNumberColumn = 8;   // Cột H
        private const int StepActionColumn = 9;   // Cột I
        private const int TestDataColumn = 10;    // Cột J
        private const int ExpectedResultColumn = 11; // Cột K
        private const int ActualResultColumn = 12;   // Cột L
        private const int ResultStatusColumn = 13;   // Cột M (Cột ghi chữ PASS/FAIL)
        private const int NotesColumn = 14;
        private static string ExcelFilePath => ResolveExcelFilePath();

        static ExcelDataProvider()
        {
            ExcelPackage.License.SetNonCommercialPersonal("BDCLPM_THFinal");
        }
        ...
        private static string ResolveExcelFilePath()
        {
            ...
            // var candidateInTestData = Path.Combine(current, "TestData", DefaultExcelFileName); if
            // (File.Exists(candidateInTestData)) { return candidateInTestData; }

            //    current = Directory.GetParent(current)?.FullName ?? "";
            //}

            //return Path.Combine(AppContext.BaseDirectory, DefaultExcelFileName);
            return "D:\\HK3\\BDCLPM\\TH\\FinalLab\\Testcase.xlsx";
        }
